...
export async function POST(req: NextRequest) {
    ...
    try {
        console.log(`[Neemo] POST received`);
        ...
        if (numMedia > 0) {
            ...
            if (selection.slug) {
                ...
                console.log(`[Neemo] Interpreted intent: ${command.intent}`);
                await runCmd(command, selection.slug, selection.shops.length > 1);
            }
        }
        ...
        console.error('[Neemo] Webhook Error Trace:', error);
